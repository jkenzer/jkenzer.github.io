...
function setup() {
  ...
  for (let i = 0; i < divnum; i++) {
    // ang = spreads the balls out, ball count, speed, height (how big the who thing is)
    let ball = new Ball(TWO_PI / divnum * i, divnum, 0.5, height * 0.2);
    balls.push(ball);
  }
}
...
class Ball {
  ...
  update() {
    let stepang = (TWO_PI + PI) / this.div;
    this.ang += radians(this.speed);

    this.positions = [];
    this.ids = [];
    // loops through the trails. The higher the posnum, the higher the tail
    for (let i = 0; i < this.posnum; i++) {
      ...
      for (let n = 0; n < nnum; n++) {
        let nstepang = stepang / nnum * n * map(sin(radians(frameCount * 0.5)), -1.0, 1.0, 0.0, 3.0);
        let npos = createVector(cos(cang + nstepang) * this.rad + width * 0.5, sin(cang + nstepang) * this.rad + height * 0.5);
        let h2 = cos(angt * TWO_PI + nstepang) * sin(angt * TWO_PI + nstepang) * this.rad * 0.1;
